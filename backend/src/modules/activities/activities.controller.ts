...
export class ActivitiesController {
  ...
  listarModelos(@Request() req: any) {
    return this.svc.listarModelos(req.tenantId);
  }

  @Get('modelos/todos')
  @Roles('admin', 'super_admin')
  listarTodos(@Request() req: any) {
    return this.svc.listarTodosModelos(req.tenantId);
  }

  @Post('modelos')
  @Roles('admin', 'super_admin')
  criar(@Request() req: any, @Body() dto: CriarAtividadeModeloDto) {
    return this.svc.criarModelo(req.tenantId, dto);
  }

  @Patch('modelos/:id')
  @Roles('admin', 'super_admin')
  atualizar(@Request() req: any, @Param('id') id: string, @Body() dto: AtualizarAtividadeModeloDto) {
    return this.svc.atualizarModelo(req.tenantId, id, dto);
  }
  ...
  excluir(@Request() req: any, @Param('id') id: string) {
    return this.svc.excluirModelo(req.tenantId, id);
  }
  ...
  listar(@Request() req: any, @Query('competencia') competencia?: string) {
    return this.svc.listarExecucoes(req.tenantId, competencia);
  }

  @Get('resumo')
  @Roles('admin', 'financeiro', 'operacional', 'consulta', 'super_admin')
  resumo(@Request() req: any, @Query('competencia') competencia?: string) {
    return this.svc.resumoExecucoes(req.tenantId, competencia);
  }

  @Post('gerar')
  @Roles('admin', 'financeiro', 'super_admin')
  gerar(@Request() req: any, @Body() dto: GerarExecucoesDto) {
    return this.svc.gerarExecucoes(req.tenantId, dto);
  }
  ...
  baixar(
    ...
    return this.svc.baixarAtividade(req.tenantId, id, dto, req.user.id);
  }

  @Post(':id/nao-aplicavel')
  @Roles('admin', 'financeiro', 'super_admin')
  naoAplicavel(@Request() req: any, @Param('id') id: string) {
    return this.svc.marcarNaoAplicavel(req.tenantId, id, req.user.id);
  }

  @Post(':id/reabrir')
  @Roles('admin', 'financeiro', 'super_admin')
  reabrir(@Request() req: any, @Param('id') id: string) {
    return this.svc.reabrirAtividade(req.tenantId, id);
  }
}
