...
export const superAdminApi = {
  ...
  atualizarStatus:  (id: string, status: string)           => api.patch(`/super-admin/tenants/${id}/status`, { status }),
  atualizarPlano:   (id: string, plano: string)            => api.patch(`/super-admin/tenants/${id}/plano`,  { plano }),
  estenderTrial:    (id: string, trial_ate: string)        => api.patch(`/super-admin/tenants/${id}/trial`,  { trial_ate }),
  ...
  impersonate:      (tenantId: string, motivo?: string)    => api.post(`/super-admin/impersonate/${tenantId}`, { motivo }),
  historicoImpersonations: (limite?: number)               => api.get('/super-admin/impersonations', { params: { limite } }),
};
...
export const reportsApi = {
  ...
  financeiroExcel: (di?: string, df?: string) =>
                     api.get('/reports/financeiro/excel',
                       { params: { data_inicio: di, data_fim: df }, responseType: 'blob' as const }),
  contratos:       ()                         => api.get('/reports/contratos'),
  contratosExcel:  ()                         =>
                     api.get('/reports/contratos/excel', { responseType: 'blob' as const }),
  ...
  maquinasExcel:   (di?: string, df?: string) =>
                     api.get('/reports/maquinas/excel',
                       { params: { data_inicio: di, data_fim: df }, responseType: 'blob' as const }),
  estoqueExcel:    ()                         =>
                     api.get('/reports/estoque/excel', { responseType: 'blob' as const }),
};
...
export const activitiesApi = {
  ...
  listar:           (competencia?: string) =>
                      api.get('/activities', { params: { competencia } }),
  resumo:           (competencia?: string) =>
                      api.get('/activities/resumo', { params: { competencia } }),
  gerar:            (competencia: string)  =>
                      api.post('/activities/gerar', { competencia }),
  baixar:           (id: string, dto?: any) =>
                      api.post(`/activities/${id}/baixar`, dto ?? {}),
...
export const catalogExportApi = {
  exportarCatalogo: () =>
    api.get('/catalog/export/pdf', { responseType: 'blob' as const }),
};
...
export const manutencaoApi = {
  ...
  concluir:  (id: string, dto: any)    => api.post(`/manutencao/${id}/concluir`, dto).then(r => r.data),
